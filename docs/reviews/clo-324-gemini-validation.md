...
Error: Shell command failed: TASK="clo-324"
BRANCH="feat/clo-324-threat-model-suite"
...
OUTPUT=$(timeout 300 gemini --skip-trust --model "$PRIMARY_MODEL" -y --sandbox \
...
if [ -z "$OUTPUT" ] || [ $(printf '%s' "$OUTPUT" | wc -c) -lt 100 ]; then
  OUTPUT=$(timeout 300 gemini --skip-trust --model "$FALLBACK_MODEL" -y --sandbox \
    --include-directories docs,src,tests \
    -p "$PROMPT" -o text 2>/tmp/lok-gemini-impl-stderr.log)
