...
- Namespace every loker-specific key with `loker.` prefix (`loker.run_id`, `loker.phase`, `loker.strategy`, `loker.attempt`, `loker.event` for the discriminator). Exception: `schema_version` is intentionally un-namespaced — it is a cross-artefact envelope key shared with the HITL canonical (pending/response) schemas, so keeping a single un-prefixed name avoids divergence between artefact families.
...
5. **`pending.schema.json` + fixtures** (HITL design §3.3) - files: `docs/schemas/pending.schema.json`, `tests/fixtures/schemas/pending/positive/{design_review,verify_failure}.json`, `tests/fixtures/schemas/pending/negative/{missing_severity,bad_timeout_format,high_severity_with_timeout,low_severity_null_timeout}.json`.
   - Required (per HITL doc §3.3): `schema_version` (==1), `run_id` (workflow slug like `"design-doc-tdd-2026-04-25-1437-a7c3"`, NOT a UUID), `workflow`, `phase`, `severity` (enum: `low`, `medium`, `high`), `opened_at` (date-time), `timeout_at` (date-time **or null** — null is required when `severity == "high"`, otherwise must be a date-time), `artefact` ({path, kind, preview_lines: integer}), `context` ({preceded_by: array, next_phase: string|null, prompt_summary: string}), `decision_options` (array of enum `{approve, reject, comment_only}`).
...
9. **`phase_result_escalating.schema.json` + fixtures** - files: `docs/schemas/phase_result_escalating.schema.json`, `tests/fixtures/schemas/phase_result_escalating/positive/{succeeded_on_medium,exhausted}.json`, `tests/fixtures/schemas/phase_result_escalating/negative/{missing_tier,unknown_tier}.json`.
