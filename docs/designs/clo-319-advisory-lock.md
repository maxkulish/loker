...
- **Lock-file lifecycle on release.** ❌ RESOLVED: On graceful release (`release()` / `drop`), truncate the lock file to 0 bytes so that `inspect` returns `Ok(None)` for unlocked phases. This avoids having consumers (T-044) interpret staleness themselves. The race with concurrent `inspect` callers is benign: `inspect` reads the file contents, and an empty file either returns `Ok(None)` or a parse error that indicates "not held"; a truncated-but-not-empty file (race during write) is handled by `inspect` treating parse failures as "not held" (lenient reading).
