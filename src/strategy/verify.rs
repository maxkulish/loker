...
/// Error surfaced by a verification hook. Distinct from a `Fail` result:
/// `Fail` means the hook ran and decided to reject; `VerifyError` means the
/// hook itself blew up (e.g. `make` not on `$PATH`).
